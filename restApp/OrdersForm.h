...
namespace restApp {
	...
	public:
		...
		OrdersForm(Form^ form)
		{
			InitializeComponent();
			vector<order> orderVector = ordersJSONuOrdersVec();

			for (order ordrr : orderVector) {
				peroArtikliConvertOrderToVec(ordrr);
				OrderItem^ item = gcnew OrderItem(ordrr,ordrr.userUName,ordrr.comment,ordrr.timeOrd, ordrr.price);
				//item.name
				if (!ordrr.processed && (ordrr.restName == getActiveUser()))
...
#pragma region Windows Form Designer generated code
		...
		{
			System::ComponentModel::ComponentResourceManager^ resources = (gcnew System::ComponentModel::ComponentResourceManager(OrdersForm::typeid));
			...
			this->label2->Location = System::Drawing::Point(365, 38);
			this->label2->Name = L"label2";
			this->label2->Size = System::Drawing::Size(335, 61);
			this->label2->TabIndex = 2;
			this->label2->Text = L"NARUDZBE";
			// 
			// backBtn
			// 
			...
			this->backBtn->Click += gcnew System::EventHandler(this, &OrdersForm::backBtn_Click);
			// 
			// ordersLayoutTable
			// 
			...
			this->VisibleChanged += gcnew System::EventHandler(this, &OrdersForm::RestaurantsForm_VisibleChanged);
